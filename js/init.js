jQuery(document).ready(function($) {
    /* FitText Settings */
    setTimeout(() => $('h1.responsive-headline').fitText(1, {minFontSize: '40px', maxFontSize: '90px'}), 100);

    /* Smooth Scrolling */
    $('.smoothscroll').on('click',(e) => {
        e.preventDefault();
	    const target = this.hash, $target = $(target);
	    $('html, body').stop().animate({'scrollTop': $target.offset().top}, 800, 'swing', () => window.location.hash = target);
	});
    ...
    /* Contact Form */
    $('form#contactForm button.submit').onclick = () => {
        $('#image-loader').fadeIn();
